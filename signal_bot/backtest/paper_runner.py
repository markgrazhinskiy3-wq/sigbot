...
class PaperRunner:
    ...
    async def _resolve_expired(self, candles_map: dict[str, list[dict]]) -> None:
        ...
        for trade in to_resolve:
            ...
            # Primary: use timestamp if available (c_time > 0 guards against
            # OTC candles that arrive with time=0 — those are skipped here).
            min_close_time = trade.entry_time + trade.expiry_sec
            resolution_candle = None
            for c in reversed(candles):
                c_time = c.get("time", 0)
                if c_time > 0 and c_time >= min_close_time:
                    resolution_candle = c
                    break

            # Fallback: no timestamps available (OTC demo candles often have time=0).
            # Use last candle only if its close price differs EXACTLY from entry
            # (same float value = same candle = cache not yet refreshed → postpone).
            # Any price difference, even sub-pip, counts as a valid resolution.
            if resolution_candle is None:
                last = candles[-1]
                if float(last["close"]) == trade.entry_price:
                    logger.debug(
                        "PaperRunner: close == entry for %s — cache not refreshed yet, postponing",
                        sym,
                    )
                    continue
            ...
            # WIN / LOSS — no DRAW case.
            # Even a sub-pip movement is a valid outcome in binary options.
            # DRAW was causing systematic false-draws when demo-server prices barely move.
